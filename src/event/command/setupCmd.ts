import {
    EmbedFieldData, Guild, GuildMember, Interaction,
    MessageActionRow,
...
export = {
    ...
    evt: async (interaction: Interaction) => {
        ...
        if(interaction.isSelectMenu() && interaction.customId === CHECK_SELECT_MENU_ID) {
            await doIfCanSetup(interaction, async () => {
...
async function doIfCanSetup(interaction: ReplyInteraction, task: (member: GuildMember) => void) {
    await doIfHasPermission(interaction, PERMISSIONS.COMMANDS.SETUP, task);
}